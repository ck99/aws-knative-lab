...
export ISTIO_VERSION=${local.istio_version}
if [ -d istio-${local.istio_version} ]; then
  rm -rf istio-${local.istio_version}
fi
EOS
    working_dir = "./k8s"
    interpreter = ["/bin/sh", "-c"]
...
EOS
  ...
  provisioner "local-exec" {
    when = "destroy"
    command = <<EOS
for i in install/kubernetes/helm/istio-init/files/crd*yaml; do ${local.kubectl_cmd} delete -f $${i}; done
cat <<ENDISTIO | ${local.kubectl_cmd} delete -f -
apiVersion: v1
kind: Namespace
metadata:
 name: istio-system
...
${local.kubectl_cmd} delete -f istio-lean.yaml
...
${local.kubectl_cmd} delete -f istio-local-gateway.yaml
